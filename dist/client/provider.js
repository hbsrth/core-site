 ...
 */
export function CoreProvider({ children, basePath = "/api/core", track = true }) {
    ...
    useEffect(() => {
        if (window.parent === window)
            return;
        if (new URLSearchParams(window.location.search).get("core-preview") !== "1")
            return;
        const stop = autoStartCorePreview(process.env.NEXT_PUBLIC_CORE_ORIGIN ?? "");
        return () => stop();
    }, []);
    useEffect(() => {
        if (!track || typeof navigator === "undefined" || navigator.doNotTrack === "1")
            return;
