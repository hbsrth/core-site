 ...
 */
import { PREVIEW_PROTOCOL, parseToFrame } from "../protocol.js";
...
export function pickCoreOrigin(list, referrer) {
    const allowed = list
        .split(",")
...
export function startCorePreview(options) {
    ...
    const collect = () => Array.from(document.querySelectorAll(`[${MARK}]`)).map((el) => ({
    ...
    const onMessage = (event) => {
        ...
        if (message.type === "core:patch") {
            const el = document.querySelector(`[${MARK}="${cssEscape(message.field)}"]`);
            if (!el)
                return;
 ...
 */
export function autoStartCorePreview(coreOrigin) {
    if (typeof window === "undefined")
        return () => { };
    if (window.parent === window)
        return () => { };
    const params = new URLSearchParams(window.location.search);
    if (params.get("core-preview") !== "1")
        return () => { };
