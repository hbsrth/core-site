...
export interface HandlerOptions {
  ...
  mapEnquiry?: (body: Record<string, unknown>) => { name?: unknown; email?: unknown; subject?: unknown; message?: unknown };
...
function formLimited(key: string) {
  const now = Date.now();
  const recent = (formHits.get(key) ?? []).filter((t) => now - t < 10 * 60 * 1000);
  recent.push(now);
  formHits.set(key, recent);
  if (formHits.size > 5000) formHits.clear();
  return recent.length > 5;
}

/* Ziyaret tamponu: 20 olayda ya da 10 sn'de bir CORE'a. */
interface VisitEvent { path: string; visitor: string; referrer: string; at: string }
...
async function flushVisits() {
  ...
  const env = readEnv();
  if (buffer.length === 0 || !configured(env)) return;
...
async function visitorHash(request: Request, siteId: string): Promise<string> {
...
export function createCoreHandlers(options: HandlerOptions = {}) {
  const honeypot = options.honeypot ?? "website";
  const map = options.mapEnquiry ?? ((b: Record<string, unknown>) => ({ name: b.name, email: b.email, subject: b.subject, message: b.message }));
  ...
  async function GET(request: Request) {
    if (action(request) !== "health") return json({ error: "Bulunamadı." }, 404);
    const env = readEnv();
    return json({
      ok: configured(env),
  ...
  async function POST(request: Request) {
    ...
    if (what === "revalidate") {
      if (!env.webhookSecret) return json({ error: "CORE_WEBHOOK_SECRET tanımlı değil." }, 503);
      const token = /^Bearer\s+(\S+)$/i.exec(request.headers.get("authorization") ?? "")?.[1] ?? "";
      if (!token || !timingSafeEqual(token, env.webhookSecret)) return json({ error: "Yetkisiz." }, 401);
      ...
      if (tags.length === 0) return json({ error: "Etiket yok." }, 400);
    ...
    if (what === "enquiry") {
      if (!configured(env)) return json({ error: "Form bağlantısı yapılandırılmadı." }, 503);
      ...
      if (typeof body[honeypot] === "string" && (body[honeypot] as string).trim()) return json({ ok: true, id: "" }, 201);
      const ip = request.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || "unknown";
      if (formLimited(ip)) return json({ error: "Çok fazla deneme; biraz sonra yeniden deneyin." }, 429);
      ...
      if (!upstream) return json({ error: "CORE'a ulaşılamadı." }, 502);
    ...
    if (what === "visit") {
      ...
      if (buffer.length >= 20 || Date.now() - lastFlush > 10_000) await flushVisits();
      else if (!flushTimer) flushTimer = setTimeout(() => void flushVisits(), 10_000);
      return new Response(null, { status: 204 });
    }
