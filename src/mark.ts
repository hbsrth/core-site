...
export function coreField(collection: string, field: string, recordId?: string, options?: { attr?: CoreAttr }): Record<string, string> {
  if (recordId === "") return {};
  const path = formatFieldPath(collection.startsWith("@") ? { collection, field } : { collection, recordId, field });
  if (!collection.startsWith("@") && !recordId) return {};
  const out: Record<string, string> = { "data-core-field": path };
  if (options?.attr) out["data-core-attr"] = options.attr;
  return out;
}

/** `**kalın**` işaretlemesini React öğelerine çevirir; başka işaretleme yorumlanmaz. */
export function inlineText(value: string): Array<string | { bold: string }> {
